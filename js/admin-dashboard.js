...
async function verDetalhesLojaAdmin(lojaId) {
    ...
    const status = STATUS_ADMIN[loja.status_aprovacao] || STATUS_ADMIN.pendente;
    const logo = loja.logo_url
        ? `<img src="${escaparAtributoAdmin(loja.logo_url)}" class="loja-logo-admin" alt="Logo da ${escaparHTMLAdmin(loja.nome || "Loja")}" loading="lazy">`
...
function renderizarItemHistoricoAdmin(item) {
    ...
    return `
        <div class="historico-item">
            <strong>${escaparHTMLAdmin(rotuloStatusAdmin(item.status_anterior))} â†’ ${escaparHTMLAdmin(rotuloStatusAdmin(item.status_novo))}</strong>
            <span>${formatarDataHoraAdmin(item.criado_em)}${administrador}</span>
