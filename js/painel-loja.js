    ...
    try {
        ...
        if (pedidos.length === 0) {
            lista.innerHTML = `
                <div class="sem-pedidos">
                    ...
                    <p>
                        Quando algum cliente fizer um pedido,
                        ele aparecerá aqui.
                    </p>
                </div>
            `;
        ...
        lista.innerHTML = `
            <div class="sem-pedidos">
                ...
                <p>Não foi possível buscar os pedidos da sua loja.</p>
                <button type="button" class="btn" onclick="carregarPedidos()">
                    <i class="fa-solid fa-rotate-right"></i>
                    Tentar novamente
                </button>
            </div>
...
function criarCardPedido(pedido) {
    ...
    const numeroPedido = obterNumeroPedido(id);
    const dataPedido = formatarDataHora(pedido.created_at);
    const pagamento = formatarPagamento(pedido.forma_pagamento);
    const statusTexto = formatarStatusPedido(status);
    const classeStatus = obterClasseStatusPedido(status);
    const acao = obterAcaoProximoStatus(status);

    const itensHTML = itens.length > 0
        ? itens.map(criarItemDetalhePedido).join("")
        : '<div class="pedido-item-vazio">Nenhum item encontrado.</div>';

    let observacoesHTML = "";

    if (pedido.observacoes && String(pedido.observacoes).trim()) {
        observacoesHTML = `
            <div class="pedido-observacoes">
                ...
                <p>${formatarTextoMultilinha(pedido.observacoes)}</p>
            </div>
        `;
    }
    ...
    if (pedido.codigo_rastreio) {
        rastreioHTML = `
            <div class="pedido-observacoes">
                ...
                <p>${escaparHTML(pedido.codigo_rastreio)}</p>
            </div>
        `;
    }
    ...
    if (status === "enviado") {
        botaoStatusHTML = `
            <button
                ...
                disabled
                title="A confirmação de entrega deve ser feita pelo cliente"
            >
        ...
        botaoStatusHTML = `
            <button
                type="button"
                class="btn btn-status-pedido"
                onclick="avancarStatusPedido('${escaparJS(id)}', '${escaparJS(status)}')"
            >
    ...
    return `
        <article class="pedido-card" data-pedido-id="${idSeguro}">
            ...
            <div class="pedido-resumo">
                <div class="pedido-resumo-item">
                    ...
                    <div>
                        ...
                        <strong>${escaparHTML(pagamento)}</strong>
                    </div>
                </div>
            ...
            <div class="pedido-acoes">
                <button
                    ...
                    onclick="alternarDetalhesPedido('${escaparJS(id)}', this)"
                >
...
function criarItemDetalhePedido(item) {
    let produto = item?.produtos;

    if (Array.isArray(produto)) produto = produto[0];
...
function alternarDetalhesPedido(pedidoId, botao = null) {
    ...
    if (botao) {
        botao.innerHTML = vaiAbrir
            ? '<i class="fa-solid fa-eye-slash"></i> Ocultar detalhes'
...
async function avancarStatusPedido(pedidoId, statusAtual) {
    ...
    const statusNormalizado = normalizarStatusPedido(statusAtual);

    // Envio sempre exige rastreio e fica concentrado
    // na página completa de pedidos da loja.
    if (statusNormalizado === "em_preparacao") {
        notificar(
    ...
    // O lojista não confirma a entrega.
    ...
    const acao = obterAcaoProximoStatus(statusNormalizado);
    ...
    const confirmou = await window.confirmarAcao({
        titulo: acao?.titulo || "Atualizar pedido?",
        mensagem: `O pedido #${obterNumeroPedido(pedidoId)} passará de "${formatarStatusPedido(statusNormalizado)}" para "${formatarStatusPedido(proximoStatus)}". Deseja continuar?`,
        textoConfirmar: acao?.textoConfirmar || "Confirmar",
        textoCancelar: "Cancelar",
        perigo: false
    });
    ...
    try {
        // ALTERAÇÃO SEGURA:
        // nenhuma atualização direta de public.pedidos é feita aqui.
        const { data, error } = await window.db.rpc(
            "atualizar_status_pedido_loja",
            {
                p_pedido_id: pedidoId,
                p_novo_status: proximoStatus,
        ...
        notificar(
            `Pedido #${obterNumeroPedido(pedidoId)} atualizado para "${formatarStatusPedido(proximoStatus)}".`,
            "sucesso",
            "Status atualizado!",
            3500
        );
...
// ==========================================
// PRÓXIMO STATUS DISPONÍVEL NO PAINEL
// ==========================================

function obterProximoStatusPedido(status) {
    const fluxo = {
        aguardando_pagamento: "pago",
        pago: "em_preparacao"
    };

    return fluxo[normalizarStatusPedido(status)] || null;
}
...
function obterAcaoProximoStatus(status) {
    ...
    return acoes[statusNormalizado] || null;
}
...
function formatarStatusPedido(status) {
    ...
    return textos[normalizado] || "Status desconhecido";
}
...
function obterClasseStatusPedido(status) {
    ...
    return classes[normalizado] || "status-desconhecido";
}
...
function formatarPagamento(pagamento) {
    ...
    return pagamentos[valor] || pagamento || "Não informado";
}
...
function tratarErroPainel(erro) {
    ...
    if (
        texto.includes("pedidos_status_check") ||
        (texto.includes("check constraint") && texto.includes("status"))
    ) {
        return "Este status não é permitido para o pedido.";
    }
